...
**Course:** CSET419 – Introduction to Generative AI  
**Experiment:** Pix2Pix GAN using a U-Net generator and PatchGAN discriminator

## Objective
[cite_start]The objective of this lab is to implement Pix2Pix GAN using a U-Net generator and PatchGAN discriminator and compare results with CNN.[cite: 424] [cite_start]It is designed to understand paired image-to-image translation using GANs.[cite: 426]

## Experiment Overview



[cite_start]In this experiment, we explore the Pix2Pix Architecture, which is a conditional GAN (cGAN).[cite: 432, 433] Unlike the baseline CNN from the previous lab, this architecture uses two competing networks. [cite_start]By training the model using adversarial + reconstruction loss[cite: 430], the discriminator forces the generator to produce sharp, realistic translations rather than the blurry averages seen in standard Encoder-Decoder models.

## Domain & Model
* **Dataset Selected:** * [cite_start]Dataset may be used Edges2Shoes.[cite: 443]
    * [cite_start]Use paired images (Edges → Real Images).[cite: 442]
    * [cite_start]Split into training and testing sets.[cite: 444]
* **Generative Model:** **Pix2Pix Architecture**
    * [cite_start]**Generator (G): U-Net**[cite: 434]
        * [cite_start]Encoder–decoder CNN with skip connections.[cite: 435]
        * [cite_start]Preserves low-level spatial features.[cite: 436]
        * [cite_start]Reduces information loss caused by pooling.[cite: 437]
    * [cite_start]**Discriminator (D): PatchGAN**[cite: 438]
        * [cite_start]Classifies local image patches as real or fake.[cite: 439]
        * [cite_start]Encourages sharpness and texture realism.[cite: 440]

## Methodology
1. **Dataset Preparation:**
    * [cite_start]Prepared the dataset using paired images (Edges → Real Images).[cite: 442]
2. **Architecture Design:**
    * [cite_start]Implemented U-Net Generator utilizing downsampling blocks with convolution and upsampling blocks with skip connections.[cite: 445, 446, 447]
    * [cite_start]Implemented PatchGAN Discriminator using convolutional layers (with no fully connected layers) to output a patch-level authenticity map.[cite: 448, 449, 450, 451]
3. **Training:**
    * [cite_start]Utilized the Adam optimizer with a learning rate of 0.0002.[cite: 453, 454]
    * [cite_start]Applied both Adversarial loss and L1 reconstruction loss.[cite: 456, 457]
    * [cite_start]Conducted alternate training of Generator and Discriminator.[cite: 458]
4. **Performance Comparison:**
    * [cite_start]Compared Pix2Pix GAN outputs with the baseline CNN Encoder–Decoder.[cite: 460, 461]

## Tools & Technologies
* **Language:** Python
* **Framework:** PyTorch
* **Library:** OpenCV (for generating Canny edge maps)

## Key Outputs
* **Training Metrics:** Real-time monitoring of both the Discriminator loss (patch authenticity) and Generator loss (fooling the discriminator + L1 pixel similarity).
* **Visual Results:** A side-by-side comparison displaying:
    * **Input (Edges):** The structural outline of the object.
    * **Output (Pix2Pix):** The image generated by the U-Net.
    * **Ground Truth:** The original target real image.
...
# Lab – 8: Create Artistic Outputs using GAN

**Course:** CSET419: Introduction to Generative AI  
**Experiment:** Generate Artistic Images using Generative Adversarial Networks (GAN)

## Objective
The objective of this lab is to generate artistic images using Generative Adversarial Networks (GAN). The generator learns to create new images from random latent vectors and produces creative visual outputs.

## Experiment Overview

In this experiment, we explore how GAN models generate new images. A GAN consists of two neural networks: a Generator and a Discriminator. The generator creates images from random noise, while the discriminator evaluates whether the images are real or fake.

Both networks compete during training. As training progresses, the generator improves its ability to create realistic images.

## Domain & Model

* **Dataset:**  
  * Any image dataset can be used.  
  * Example datasets include CIFAR-10, artwork datasets, or landscape images.  
  * Images from the dataset are used to train the GAN model.

* **Generative Model:** **Deep Convolutional GAN (DCGAN)**  
  * The generator converts random latent vectors into images.  
...
2. **GAN Model Implementation**
   ...
   * Train the GAN model using adversarial learning.

3. **Latent Space Exploration**
   * Generate images using random latent vectors.
   * Interpolate between two latent vectors to observe smooth transitions.
...
Upon successful execution, the program should display generated artistic images.

Students should observe:

* New images generated by the GAN generator.
* Artistic variations from different latent vectors.
* Smooth transitions during latent vector interpolation.
* Creative patterns learned from the training dataset.
